...
public class MondayFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        //TextView abc = (TextView) findViewById();
        ...
        com.homemade.akhilez.timetable.Table.timeString=com.homemade.akhilez.timetable.Table.getTime();

        com.homemade.akhilez.timetable.Table.dayString="Mon";
        Table.fragmentDay=Table.dayString;
       // com.example.akhilez.timetable.Table.timeString="1040";
        com.homemade.akhilez.timetable.Table.timeInt=Integer.parseInt(com.homemade.akhilez.timetable.Table.timeString);
        //com.example.akhilez.timetable.Table.timeInt=410;
        com.homemade.akhilez.timetable.Table.dayInt=com.homemade.akhilez.timetable.Table.getDayInt(com.homemade.akhilez.timetable.Table.dayString);
        int curPeriodInt=com.homemade.akhilez.timetable.Table.bring(com.homemade.akhilez.timetable.Table.timeInt);
        ...
        if (Table.dayString.equals(Table.getDay())){
            ...
            headingText.setTextColor(Color.parseColor(Table.colors[0]));
            headingText.setPadding(dpToPx(25),0,0,0);
            headingLayout.setPadding(0, 0, 0, dpToPx(10));
        ...
        else if(Table.getDay().equals("Tue")){
            TextView headingText = new TextView(this.getContext());
        ...
        if(Table.getDay().equals("Sun")/*&&Table.getTime24hr()>1630*/)ViewPagerContents.setTomTimer(relative);


        if(Table.getDay().equals(Table.fragmentDay)){
            if(curPeriodInt>=0 && curPeriodInt<=8) {
                ViewPagerContents.highlight(relative[curPeriodInt],periods[curPeriodInt],times[curPeriodInt],Table.colors[0]);
